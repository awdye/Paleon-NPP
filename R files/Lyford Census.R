Lyf.cen<-read.csv("Lyford Census.csv",header=TRUE)
...
annAB<-aggregate(AB~year,sum,data=lyf.plot)
annAB$AB<-annAB$AB/3
...
lyf.plot$annAB<-lyf.plot$annAB*(10000/929)
...
#random for annAB
randomLyf<-replicate(10000,lyfordAB(subset(lyf.plot,plot %in% sample(unique(lyf.plot$plot),4))))
...
errbar(Lyf.randomStats$year,Lyf.randomStats$annMean,Lyf.randomStats$upper95,Lyf.randomStats$lower95,
       xlim=c(1970,2011),ylim=c(1,6),ylab="AB(Mg/ha/yr)",xlab="year")
par(new=TRUE)
plot(LyfordNPP$year,LyfordNPP$LyfordMean,type="l",col="forestgreen",lwd=2,xlim=c(1970,2011),ylim=c(1,6),ylab="AB(Mg/ha/yr)",xlab="year",
     main="Lyford annual NPP")
lines(LyfordNPP$year,LyfordNPP$max,lty=3,lwd=1.5)
lines(LyfordNPP$year,LyfordNPP$min,lty=3,lwd=1.5)
legend(2002.3,1.4,paste(c("Tree ring","Census","Ann. max/min")),col=c("forestgreen","chocolate","black"),lty=c(1,1,3),
       lwd=c(1.5,1.5,1.5),
       cex=.6,bty="n")

#total AB over time
plot(HowNPPnest$year,HowNPPnest$meanAnn,type="l",col="forestgreen",lwd=2,xlim=c(1970,2010),ylim=c(2,6),ylab="AB(Mg/ha/yr)",xlab="year",
     main="Lyford NPP")
par(new=TRUE)
errbar(randomStats$year,randomStats$annMean,randomStats$upper95,randomStats$lower95,
       xlim=c(1990,2010),ylim=c(0.5,4),ylab="AB(Mg/ha/yr)",xlab="year")
lines(PlotsWide$year,PlotsWide$max.ann,lty=3,lwd=1.5)
lines(PlotsWide$year,PlotsWide$min.ann,lty=3,lwd=1.5)
legend("bottomright",paste(c("Tree ring","Census","max/min")),col=c("forestgreen","chocolate","black"),lty=c(1,1,3),
       lwd=c(1.5,1.5,1.5),
       cex=.6,bty="n")
