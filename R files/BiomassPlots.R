...
LyfAnn<-ggplot(data=TPNPP,aes(Year,NPP))+
  ...
  #geom_ribbon(aes(ymin=NPPmin,ymax=NPPmax),alpha=0.2,fill="forestgreen")+
 MCtheme

#total AB
LyfTot<-ggplot(data=ab.InnerAnn,aes(year,annABTr))+
...
ggplot(bioSpPlot,aes(x=PlotAB/2,y=AB,fill=factor(species),width=PlotAB),width=50,color=NA)+
...
TRsite<-aggregate(AB~site+species,data=bioSpPlot,mean)

CenSp<-ggplot(CenSpAll2,aes(x=1,y=AB,fill=factor(species)),color=NA)+
  geom_bar(stat="identity",position="fill")+
  coord_polar(theta="y")+
  facet_wrap(~site,ncol=1)+
  scale_fill_manual(values=palette3,guide=guide_legend(title="species"))+
  labs(x="",y="",title="Permanent Plots")+
  #geom_text(aes(x=0,y=80,label=paste(("AB = "),TotalAB,(" Mg/ha"))),size=3)+
  theme_bw()+theme(legend.title=element_text(size=15),legend.text=element_text(size=15),panel.border=element_blank(),panel.grid.major=element_blank(),panel.grid.minor=element_blank(),
                   panel.background=element_rect(fill="transparent"),plot.background=element_rect(fill="transparent"),
                   axis.line=element_blank(),axis.text.x=element_blank(),plot.title=element_text(size=25),
...
CenDist<-ggplot(cenDist,aes(dbh))+geom_histogram(aes(y=..density..),color="NA",fill="burlywood",alpha=0.2)+geom_density(color="burlywood")+
...
field.all<-arrange(transform(field.all,site=factor(site,neworder2)),site)
...
ferNPP<-subset(ferNPP,ferNPP$year<=2003&ferNPP$year>=2000)



#Boxplot comparisons
NPPall<-read.csv("NPPall.csv")
Lyfpp<-read.csv("LyfPP.csv")
Lyfpp$Method<-"Permanent Plot"
Lyfpp$Comparison<-"Permanent Plot"
Lyfpp$site<-"Tower"
NPPall<-rbind(NPPall,Lyfpp)

library("plyr")
library("grid")
library("scales")
siteOrd<-c("Howland","Lyford","Tower","Fernow")
...
palette<-c("forestgreen","burlywood","red","orange2")

ggplot(NPPall)+
  geom_boxplot(aes(Comparison,NPP,fill=Method))+scale_fill_manual(values=alpha(palette,.5))+
